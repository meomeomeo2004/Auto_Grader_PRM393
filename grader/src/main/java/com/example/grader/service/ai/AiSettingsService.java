 ...
 *
 * <p>VAI GIẢNG VIÊN, và nhãn phải nằm ở TỪNG BEAN của tầng {@code service.ai} chứ không chỉ ở
 * controller: lớp này có {@code @PostConstruct} đọc bảng {@code ai_settings} ngay lúc khởi động.
 * Chỉ cắt controller thì bản người chấm vẫn nạp cả tầng này và vẫn chạm vào cơ sở dữ liệu của nó —
 * soạn đề bằng AI là việc của người ra đề, không thuộc sản phẩm của người chấm.
 */
@Slf4j
